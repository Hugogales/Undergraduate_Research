...
class HUGO: # Hierarchical Unified Generalized Optimization
    ...
    def select_action(self, state):
        try :
            state_tensor = torch.FloatTensor(state).unsqueeze(0).to(self.device) # [num_agents, state_size]
            with torch.no_grad():
    ...
    def get_actions(self, states):
        ...
        actions_in, log_probs, entropies = actions_in.squeeze(0).cpu().numpy(), log_probs.squeeze(0).cpu().numpy(), entropies.squeeze(0).cpu().numpy()
        for i, state in enumerate(states):
            ...
            actions.append(self._action_to_input(actions_in[i]))
